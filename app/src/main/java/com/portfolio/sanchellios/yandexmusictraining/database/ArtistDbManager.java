...
public class ArtistDbManager {
    ...
    public void saveArtistToDb(Artist artist){
        ...
        values.put(ArtistTable._ID, artist.getId());
    ...
    public ArrayList<Artist> getArtistsFromDb(){
        ...
        String query = "SELECT "+ ArtistTable._ID +
                " FROM " + ArtistTable.TABLE_NAME + ";";
        Cursor cursor = db.rawQuery(query, null);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()){
            artists.add(getArtistFromDb(cursor.getInt(cursor.getColumnIndex(ArtistTable._ID))));
            cursor.moveToNext();
        }
        cursor.close();
        return artists;
    ...
    private Artist getArtistFromDb(int artistId){
        String query = "SELECT * FROM " + ArtistTable.TABLE_NAME +
                " WHERE " + ArtistTable._ID + "= '" + artistId +"';";
        Cursor cursor = db.rawQuery(query, null);
        cursor.moveToFirst();
    ...
    private Artist getArtist(Cursor cursor){
        ...
        id = cursor.getInt(cursor.getColumnIndex(ArtistTable._ID));
